...
genfuncast::view_header($in{ss}, $s3s[2], 'Terima Inventori');
...
{ $sukses=0;
 ...
 {
#print("INSERT INTO KIRIMH (RECID, NOSURAT, DTKIRIM, CABANG_TUJ, IDLAMA_TUJ, OPRCREATE, JENIS, KET) VALUES
#                    ($recid, '$nosurat', '$in{tgtrm}', '$in{tujuan}', '$idlama', '$s3s[0]', 'T','$in{ket}');");

  $q1 = $dba1->do("INSERT INTO KIRIMH (RECID, NOSURAT, DTKIRIM, CABANG_TUJ, IDLAMA_TUJ, OPRCREATE, JENIS, KET) VALUES
                    ($recid, '$nosurat', '$in{tgtrm}', '$in{tujuan}', '0', '$s3s[0]', 'T','$in{ket}');");
     ...
     {   if($in{"kodeinv$i"})
         { $jml++;
           ...
           '".$in{"kondisiasal$i"}."','".$in{"ketasal$i"}."');");
             ...
             $m=substr "0$m",-2;
             $periode = $y."-".$m;
           if($q2!=0){ $q3 = $dba1->do("update stok set terima=terima+1, oprupdate='$s3s[0]' where jenisinv='$jenisinv';"); }

           if($q3!=0)
           { $q3 = $dba1->do("update inventori set kondisi='".$in{"kondisi$i"}."',keterangan='".$in{"keterangan$i"}."',
              currcabang='$in{tujuan}', curridlama='0', oprupdate='$s3s[0]' where kode='".$in{"kodeinv$i"}."';");
             if($q3!=0){ $qsukses++; }
           }
         }
     }
     if($qsukses==$jml){ $warning = "<div class=warning_ok>Sukses Simpan Terima Inventori dengan Nomor Surat : '$nosurat'  </div>"; $sukses=1;  }
     else
     { $dba1->do("delete from kirimd where kirimid=$recid; ");
       $dba1->do("delete from kirimh where recid=$recid; ");
       $warning = "<div class=warning_not_ok>GAGAL Simpan Terima Inventori   </div>"; $sukses=0;
     }
  }
...
print qq~
     ...
     <td align=center> <h2 class="hurufcol">Terima Inventori</h2> </td>
...
<input type=hidden name=pages value=at0013>
<table>
...
if ($in{cabang}){$subq=" and i.currcabang='$arr[0]'"; }

#$subq = substr $subq,5;
#if ($subq) {$subq = "where $subq ";}

$q = $dba1->prepare("
select namaklp,j.namajenis,i.kode,m.namamerek,t.namatipe,u.namaukuran,w.namawarna,i.dtlastkirim,
...
{ $trm="";
  ...
  <tr bgcolor=$colcolor height=20>
     ...
     <td align='left' class="hurufcol" valign="top"><input type=text name=keterangan$no value='$row[9]' class=huruf1 maxlength="30"></td>
     <td align='left' class="hurufcol" valign="top">~;
...
<input type=hidden name=cabangasal value='$arr[0]'>

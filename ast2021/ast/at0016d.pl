...
if ($in{simpan})
{
# print("update opnameh set cabapp='Y',cabopr='$s3s[0]',dtaud=current_timestamp, oprupdate='$s3s[0]' where recid='$in{recido}';");
$q1 = $dba1->do("update opnameh set audapp='Y',audopr='$s3s[0]',dtaud=current_timestamp, oprupdate='$s3s[0]' where recid='$in{recido}';");
 }
if($q1!=0)
     {
      $jml=0;
 ...
 for ($i=1; $i<=$in{baris}; $i++)
       { if($in{"kodeinv$i"})
         { $jml++;
           @item=split(/\-/,$in{"kodeinv$i"});
           
$q2 = $dba1->do("update inventori set kondisi='".$in{"kondisi$i"}."',keterangan='".$in{"ket$i"}."',oprupdate='$s3s[0]' where kode in (select kodeinv from opnamed where kodeinv='".$in{"kodeinv$i"}."');");

# print("update inventori set kondisi='".$in{"kondisi$i"}."',keterangan='".$in{"ket$i"}."',oprupdate='$s3s[0]' where kode in (select kodeinv from opnamed where kodeinv='".$in{"kodeinv$i"}."');");

    if($q2!=0){ $warning = "<div class=warning_ok>Sukses Approval </div>"; }
...
<form method=post action="/cgi-bin/sisasset.cgi" >
      ...
      <input name="pages" type="hidden" value=at0016>
...
<form action="/cgi-bin/sisasset.cgi" method="post" name='add'>
  ...
  <input type=hidden name=pages value=at0016d>
...
~;
 ...
 oldidlama, oldkondisi, oldket,d.kondisi, d.ket
 ...
 order by nik, namaklp");
  ...
  {  $oldpemilik=""; $oldkondisi=""; $oldket="";
    ...
    <tr bgcolor=$colcolor height=20>
     ...
     <td align='left' class="hurufcol" valign='top'>$row[10]<br/>$oldket
     <input type=hidden name=kodeinv$no value='$row[1]'>
     <input type=hidden name=kondisi$no value='$row[15]'>
     <input type=hidden name=ket$no value='$row[16]'>

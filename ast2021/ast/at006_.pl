...
genfuncast::view_header($in{ss}, $s3s[2], 'Master Tipe');
...
print qq~
     ...
     <td align=center> <h2 class="hurufcol"> MASTER TIPE</h2> </td>
     ~;
if ($s3s[11] eq 'S')
{     print qq~<td align=right width=100>
      <!--<a href='/cgi-bin/colorbox.cgi?proses=str000&display=at006a' class='iframe'>Add Tipe</a>-->
      
      <form method=post action="/cgi-bin/sisasset.cgi" >
      <input type="submit" name="back" value="Add Tipe" class="huruf1"/>
...
<script type='text/javascript' src='/jquery.min.js'></script>
  ...
  <tr bgcolor="$dark">
   ...
   <td align='center' class="hurufcol" width=100> Tipe</td>

   <td align='center' class="hurufcol" width=370> Nama Tipe </td>
   <td align='center' class="hurufcol" width=120> Action</td>
  </tr>
~;

$q = $dba1->prepare("select tipe,namatipe from tipeinv $subq order by tipe,namatipe ");
$q->execute();
$no=1;
$tmpkat = '';

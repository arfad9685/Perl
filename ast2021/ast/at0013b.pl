...
");
  ...
  {  $pemilik=""; $kondisi_asal=""; $ket_asal="";
    ...
    <tr bgcolor=$bg height=20>
     ...
     <a class=huruflink href="asetbox.cgi?display=at0010b&kd=$row[1]&ss=$in{ss}&from=at0013b&rid=$in{rid}">$row[1]</a></td>
    ...
    {  $q = $dba2->prepare("select nosurat, h.recid from kirimh h, kirimd d where h.recid=d.kirimid and kodeinv='$row[1]' and kembaliid=$in{rid}");
       $q->execute();
       @row2 = $q->fetchrow_array();
       print qq~<td align='center' class="hurufcol" width=80>
       <a class=huruflink href="asetbox.cgi?display=at0017b&sid=$row2[1]&ss=$in{ss}&from=at0013b&rid=$in{rid}">$row2[0]</a></td>~;
    }

...
genfuncast::view_header($in{ss}, $s3s[2], 'Master Kelompok Jenis');
genfuncast::validasi_akses($s3s[11], $in{ss});

if ($s3s[11] eq 'S' and $in{kd}  && $s3s[7]==0)
{ $postall = $dba1->do("DELETE FROM KLPJENISINV  WHERE KLPJENIS='$in{kd}'");
        if($postall!=0)
        { $warning_del = "<div class='warning_ok'>Sukses Hapus Jenis</div><br/> ";}
        else { $warning_del = "<div class='warning_not_ok'>Gagal Hapus Jenis</div><br/> ";}
}
...
print qq~
     ...
     <td align=center> <h2 class="hurufcol"> MASTER KELOMPOK JENIS</h2> </td>
     ~;
if ($s3s[11] eq 'S')
{     print qq~<td align=right width=100>
      ...
      <input name="pages" type="hidden" value=at007a>
...
{  with(document.mst)
 ...
 $warning_del
  ...
  <input type=hidden name=pages value=at007>
...
<script type='text/javascript' src='/jquery.min.js'></script>
  ...
  <tr bgcolor="$dark">
   ...
   <td align='center' class="hurufcol" width=100> Kode Kelompok Jenis </td>
   <td align='center' class="hurufcol" width=370> Nama Kelompok Jenis </td>
   <td align='center' class="hurufcol" width=120> Action</td>
  </tr>
~;

$q = $dba1->prepare("select klpjenis,namaklp from klpjenisinv  $subq order by klpjenis,namaklp ");
$q->execute();
$no=1;
$tmpkat = '';

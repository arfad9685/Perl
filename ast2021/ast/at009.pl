...
genfuncast::view_header($in{ss}, $s3s[2], 'Master Ukuran Inventori');
genfuncast::validasi_akses($s3s[11], $in{ss});

if ($s3s[11] eq 'S' and $in{kd}  && $s3s[7]==0)
{ $postall = $dba1->do("DELETE FROM UKURANINV WHERE UKURAN='$in{kd}'");
        if($postall!=0)
        { $warning_del = "<div class='warning_ok'>Sukses Hapus Ukuran</div><br/> ";}
        else { $warning_del = "<div class='warning_not_ok'>Gagal Hapus Ukuran</div><br/> ";}
}
...
print qq~
     ...
     <td align=center> <h2 class="hurufcol"> MASTER UKURAN INVENTORI</h2> </td>
     ~;
if ($s3s[11] eq 'S')
{     print qq~<td align=right width=100>
      <!--<a href='/cgi-bin/colorbox.cgi?proses=str000&display=at009a' class='iframe'>Add Ukuran Inv</a>-->

      <form method=post action="/cgi-bin/sisasset.cgi" >
      <input type="submit" name="back" value="Tambah" class="huruf1"/>
      <input name="pages" type="hidden" value=at009a>
...
{  with(document.mst)
 ...
 $warning_del
  ...
  <input type=hidden name=pages value=at009>
...
<script type='text/javascript' src='/jquery.min.js'></script>
  ...
  <tr bgcolor="$dark">
   ...
   <td align='center' class="hurufcol" width=200> Kode Ukuran Inventori </td>
   <td align='center' class="hurufcol" width=370> Nama Ukuran Inventori</td>
   <td align='center' class="hurufcol" width=120> Action</td>
  </tr>
~;

$q = $dba1->prepare("select ukuran,namaukuran from ukuraninv $subq order by ukuran ");
$q->execute();
$no=1;
$tmpkat = '';

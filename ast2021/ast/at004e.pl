...
genfuncast::view_header($in{ss}, $s3s[2], 'Edit Menu');
...
~;
$query = $dba1->prepare("select xid,  namaprog, ex_in from  X0000002  WHERE  namaexe='$in{kd}' ");
$query->execute();
...
<form action="/cgi-bin/sisasset.cgi" method="post" name='add'>
  ...
  <input type=hidden name=pages value=at004e>

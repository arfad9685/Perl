...
genfuncast::view_header($in{ss}, $s3s[2], 'Detail Serah Inventori');
...
");
  ...
  {  $pemilik=""; $kondisi_asal=""; $ket_asal="";
    ...
    <tr bgcolor=$bg height=20>
     ...
     <td align='center' class="hurufcol" valign='top'><a class=huruflink href="sisasset.cgi?pages=at0010d&kd=$row[1]&ss=$in{ss}&from=at0017d&sid=$in{sid}">$row[1]</a></td>
     ...
     { if($row[14])
       { $q = $dba2->prepare("select  nosurat from kirimh where recid='$row[14]'");
         ...
         <a class=huruflink href="sisasset.cgi?pages=at0013d&rid=$row[14]&ss=$in{ss}&from=at0017d&sid=$in{sid}">$row2[0]</a></td>~;
       }

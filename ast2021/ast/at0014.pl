...
genfuncast::view_header($in{ss}, $s3s[2], 'Opname');
...
print qq~
    ...
    <td align=right width=100>&nbsp; </td>
     <td align=center> <h2 class="hurufcol"> Daftar Opname </h2> </td>
     ~;
if ($s3s[11] eq 'S')
{     print qq~<td align=right width=100>
      <form method=post action="/cgi-bin/sisasset.cgi" >
      <input type="submit" name="back" value="Create Opname" class="huruf1"/>
      <input name="pages" type="hidden" value=at0014c>
...
<form action="/cgi-bin/sisasset.cgi" method="post" name='vq'>
  ...
  <input type=hidden name=pages value=at0014>
<table>
...
<script type='text/javascript' src='/jquery.min.js'></script>
  ...
  <tr bgcolor="$dark">
   ...
   <td align='left' class="hurufcol" width=100> Tanggal Audit   </td>
...
while (@row = $q->fetchrow_array())
{
  ...
  <tr bgcolor=$colcolor height=20>
     ...
     <td align='left' class="hurufcol" valign='top'><a class=huruflink href="sisasset.cgi?pages=at0014d&rid=$row[10]&ss=$in{ss}">$row[0]</a></td>

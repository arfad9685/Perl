sub acct_jur_pos {
 ...
 require "/opt/sarirasa/cgi-bin/assets/control/c_acct_jur_pos.pl"; 
 
 style_module::use_warning();
 style_module::use_calendar();
 
 xml_module::view_nsubtable("ho/tb_acct_jur_pos2","ss=$in{ss}","nostb",1);

 if ($in{tgl1} ) {
  #print $in{tgl1};
   xml_module::view_table("ho/tb_acct_jur_pos","ss=$in{ss}&tgl1=$in{tgl1}",1); #(script_view.pl, params, table cnt)
 ...
 if ($in{post}) {
  ...
  <table width=1000>
    ...
    <td width=500 align=left>
            <table width=100%>           
             <form method=GET>
             <input name="pages" type="hidden" value=acct_jur_pos>
 ...
 <script type="text/javascript"> 
   function tab_view1(data) {
      ...
      for (x in data) {
           ...
           txt +="<td align=left>&nbsp<a href=# onClick=subtable1('"+data[x].kode+data[x].tgljur+"','') style='cursor: pointer; cursor: hand;'>"+data[x].nama+"</a></td>";
           txt +="</tr>";
           txt +=" <input name=noreff"+x+" type=hidden value="+data[x].kode+data[x].tgljur+">";
      }
      txt +="</tbody></table>";        
   ...
   function tab_views1(datas) {
      ...
      txt += "<thead>";
      txt += "<tr>";
      txt += "<th align='center' colspan='4'>"+datas[1].store+"</th>";

      txt += "</tr>";
      txt += "<tr>";
      txt += "<th align='center' width=30%>Keterangan</th>";
      txt += "<th align='center' width=10%>No Coa</th>";
      txt += "<th align='center' width=30%>Debit</th>";
      ...
      txt +="<input name=cab type=hidden value="+datas[x].store+">";
      txt +="</tbody></table>";

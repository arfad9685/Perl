...
# }
 if ($in{tgl1}) {
   $cond=$cond." AND a.opn_date <=$in{tgl1} ";
 }
...
$str= qq~ [\n~; 
 $tbl_brg=$dbh->prepare("select pg.group_nama,a.brg_id, p.brg_nama, a.satuan,a.opn_date,a.opname as awal,
						k.jml_masuk,k.jml_beli,k.jml_kirim,sum(k.jml_waste) as waste,sum(k.jml_waste) as wasteWIP,sum(k.jml_waste) as wasteFG,
						'' as returWIP,'' as returFG,'' as wastenon,sum(w.qty_waste) as autowaste,sum(k.jml_nonsal) as nonsal,sum(k.jml_jual) as jual,
						(a.opname)+k.jml_masuk+k.jml_beli-k.jml_kirim - (k.jml_waste)- (k.jml_nonsal) - k.jml_jual as saldoHit,
						a.oh as saldokom, a.opname as hasilso, a.opname - a.oh as devaskhir,'' as adjus,a.opname+k.jml_masuk+k.jml_beli-k.jml_kirim - k.jml_waste- k.jml_nonsal - k.jml_jual
        as devasli
from t_opn_audit a
...
left outer join t_waste w on w.brg_id=k.brg_id and w.recid=k.recid
where 
...
k.jml_waste,k.jml_nonsal, k.jml_jual ,w.qty_waste
order by a.opn_date desc");

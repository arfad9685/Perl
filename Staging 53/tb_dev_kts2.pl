...
$str= qq~ [\n~; 
 $tbl_brg1=$dbh->prepare("select pg.group_nama,a.brg_id, p.brg_nama, a.satuan,a.opn_date,sum(a.opname) as awal,
...
while (@rec=$tbl_brg1->fetchrow())
{
   ...
   $str.= qq~ { "group_nama" : "$rec[0]", "brg_id" : "$rec[1]", "brg_nama" : "$rec[2]", "satuan" : "$rec[3]"},~ ;

...
$qrdt1=$dbh->prepare("select max(opn_date) from t_opn_audit where opn_date<='$in{tgl1}' and store_id='$in{cab}'");
  ...
  $query=" and p.brg_group in ('15','21')";
  $tol="a.DW ";
 } else {
  $query=" and p.brg_group not in ('15','21')";
  $tol="a.INV ";
 }
  
  
$qtoleransi=$dbh->prepare("select a.toleransi, $tol  from m_toleransi a,m_cab c
							where a.skala= c.store_skala
							and a.brand=c.store_brand
							and c.store_id='$in{cab}'");
$qtoleransi->execute();
@rect=$qtoleransi->fetchrow();
$rtoleransi=$rect[1]*0.01*$rect[0]*1000000; 
 ...
 $tbl_brg=$dbh->prepare("select p.brg_tabid, p.brg_nama, opn_akhir.satuan, opn_akhir.brg_id, opn_akhir.opn_date,
  ...
  avr_price hrg2, p.toleransi_dev
...
and a.menu_id is not null");
 ...
 {	
	...
	$autowsttotal=$autowsttotal+$autostore;
	$totalauto=$autowsttotal;
	
 $total=$devxfc+$autostore+$wstkrystore+$wstgumpilstore+$wstwipstore+$wstfgstore ;
 
 $totalbefore=$total;
 $totalbefore1=$totalbefore;
 $totbefore1=$totbefore1+$totalbefore1;
 $totbefore=$totbefore1;	
	
	#totaladj=$totaladj+$rec[20];
	#ttladj=$totaladj;
	
	$toleransidev=$rec[20]*$total/100;
...
$ttlfinal=$totalfinal;
 ...
 if ($totalbefore!=0) {$totalbefore = style_module::skkkff($totalbefore) } else {$totalbefore='0'};
 if ($totbefore!=0) {$totbefore = style_module::skkkff($totbefore) } else {$totbefore='0'};
   ...
   "kodetotal" : "$kodetotal","autowaste" : "$autostore","total" : "$total","totaljml" : "$totaljml",
   "wastegumpil" : "$wstgumpilstore","wastekry" : "$wstkrystore","wastewipxfc" : "$wstwipstore",
   "wastefgxfc" : "$wstfgstore","namatotal" : "$namatotal","sattotal" : "$sattotal","tgltotal" : "$tgltotal","tglopn1" : "$tglopn1",
   "totalfc" : "$totalfc","totaldev" : "$totaldev","totaldevxfc" : "$totaldevxfc","totalauto" : "$totalauto","recid" : "$rec[16]",
   "totalall" : "$totalall","totalgumpil" : "$totalwastegumpil","totalkry" : "$totalwastekry", "totalwip" : "$totalwastewip",
   "totalfg" : "$totalwastefg", "kategori" : "$kat", "jenis" : "$rec[18]","days" : "$days","toleransi" : "$toldays",
   "totalfinal" : "$ttlfinal","totfinal" : "$totalfinal", "toleransidev" : "$toledev","totaltoleransidev" : "$ttltoldev",
   "totalbefore" : "$totalbefore", "totbefore" : "$totbefore"},~ ;
   }

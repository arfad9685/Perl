...
xml_module::popup_view("800 px","85%","/cgi-bin/colorbox.cgi?pages=sis_prod_02d&ss=$in{ss}&lok_prg=$in{lok_prg}&menuid=$menu[0]&jenis=$in{jenis}&resto=$in{resto}",1); #($width, $height,$url,$key)
...
if ($in{cari}) {
 ...
 xml_module::konfirm();
 xml_module::view_table("ho/tb_prod2","$str",1); #(script_view.pl, params, table cnt)
...
print qq~
    ...
    <tr><td width=100%>
            <table width=100%>           
             <form method=GET>
             <input name="pages" type="hidden" value=sis_prod_02>
          ...
          if (chk_brand($s3s[5]))
          {
             print qq~<input name="resto" type="hidden" value="$s3s[5]">~;
          } else {   
            print qq~                        
            <tr>
...
<script type="text/javascript"> 
   function tab_view1(data) {
      ...
      if ($in{jenis} ne 'FG') {
        ...
        txt += "<th align='center'>Sat Krm</th>";
        txt += "<th align='center'>=</th>";        
        txt += "<th align='center'>Sat Stk</th>";~;
      } else {
         print qq~
        txt += "<th align='center'>Dapur</th>";
        txt += "<th align='center'>Kategori</th>";
        txt += "<th align='center'>Harga Jual</th>";
        txt += "<th align='center'>Tgl Berlaku</th>";      
        txt += "<th align='center'>Tgl Expired</th>";~;         
      }
      print qq~
      txt += "</tr>";
      txt += "</thead>";
     ...
     for (x in data) {
           txt += "<tr class=huruf2><td align=left width=100>&nbsp;"+data[x].kode+"</td><td align=left>&nbsp;";
           txt += "<span onclick=show1('kodebrg="+data[x].kode+"')><a href=#>"+data[x].nama+"</a></span></td><td align=left>&nbsp;"+data[x].satuan+"</td>";~;
           
           if ($in{jenis} ne 'FG') {
            print qq~
             txt += "<td align=right>"+data[x].konv1+"&nbsp;</td><td align=left>&nbsp;"+data[x].satkrm+"</td>";
             txt += "<td align=right>"+data[x].konv2+"&nbsp;</td><td align=left>&nbsp;"+data[x].satstr+"</td>";~;            
           } else {
            print qq~
            txt += "<td align=left>&nbsp;"+data[x].kat+"</td><td align=right>"+data[x].ht+"&nbsp;</td>";
            txt += "<td align=left>&nbsp;"+data[x].satkrm+"&nbsp;</td><td align=left>&nbsp;"+data[x].satstr+"</td>";~;
           }
           print qq~ 
           txt += "</tr>";
     }

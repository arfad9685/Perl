sub acct_rep_jns {
 ...
 if ($in{tgl1} ) {
  print $in{tgl1};
   xml_module::view_table("ho/tb_acct_jur_pes","ss=$in{ss}&tgl1=$in{tgl1}",1); #(script_view.pl, params, table cnt)
 ...
 if ($in{post}) {
  ...
  <table width=1000>
    ...
    <td width=500 align=left>
            <table width=100%>           
             <form method=GET>
             <input name="pages" type="hidden" value=acct_jur_pes>
            ...
            <tr>              
              <td align="left" bgcolor=$colcolor class="hurufcol">&nbsp;Tanggal </td>
                <td class=hurufcol>~;style_module::input_calendar('tgl1',$in{tgl1},3);
                style_module::input_calendar('tgl2',$in{tgl2},4);print qq~

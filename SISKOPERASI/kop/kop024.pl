...
genfunckop::view_header_kop($in{ss}, $s3s[2], 'Transaksi Minus', $s3s[17]);
...
\$(document).ready(function(){
   \$(".iframe").colorbox({
    ...
    \$("#click").click(function(){
     ...
     <td align=center> <h2 class="hurufcol">TRANSAKSI MINUS </h2> </td>
      <td align=right width=50>~;
if ($s3s[11] eq 'S')
{     print qq~
      <a class='iframe' href='/cgi-bin/kopbox.cgi?display=kop024c&ss=$in{ss}'><img src='/images/add.png'></a>
       ~;
}     print qq~
      </td>
     </tr>
...
<form action="/cgi-bin/siskop.cgi" method="post" name='vq'>
  ...
  <input type=hidden name=pages value=kop024>
<table>
...
if($in{reciddel})
{
     $q1 = $dbk->do("update Minus set batal='Y', oprupdate='$s3s[0]' where recid = '$in{reciddel}';");

     if($q1!=0)
     { print qq~<div class=warning_ok>Sukses Batalkan Transaksi Minus </div>~;}
     else
     { print qq~<div class=warning_ok>Gagal Batalkan Transaksi Minus </div>~;}
}
...
<script type="text/javascript">
function toDelete(n)
{  var result = confirm("Yakin Batalkan Transaksi Minus ?");
        if (result==true)
        { with(document.ps)
          {  reciddel.value = n;
             submit();
...
<form action="/cgi-bin/siskop.cgi" method="post" name="ps">
  ...
  <input type=hidden name=reciddel value=''>
     ...
     $query = $dbk->prepare("select periode, dttransaksi, keterangan, jumlah, batal, recid from Minus $subq order by dttransaksi, jumlah ");

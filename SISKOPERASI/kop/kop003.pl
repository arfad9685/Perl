...
<ul id="jsddm">
<li><a href="/cgi-bin/siskop.cgi"> <strong>LOGOUT</strong></a></li>

~;

if ($in{ubah} eq 'Change') {
 if ($in{oldpass} ne $s3s[1]) {
 ...
 if (!$erruser) {
   $rowdo=$dbp->do("UPDATE x0000003 set npassword='$in{newpass}' where namausr='$s3s[0]'");
   if ($rowdo)
   {
...
<center><p>
    ...
    <input type="hidden" name="pages" value="kop003" />

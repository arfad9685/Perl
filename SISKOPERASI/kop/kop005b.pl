...
genfunckop::view_header_kop($in{ss}, $s3s[2], 'Add Submenu', $s3s[17]);
...
{  $in{child}=~ s/\'/\ /g;
   ...
   {    $query = $dbk->prepare("select child from X0000005 where child='$in{child}'");
        $query->execute();
...
<form action="/cgi-bin/siskop.cgi" method="post" name='add'>
  ...
  <input type=hidden name=pages value=kop005b>

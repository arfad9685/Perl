...
genfunckop::view_header_kop($in{ss}, $s3s[2], 'Tambah Pencairan SIMPANAN', $s3s[17]);
...
print qq~
     ...
     <td align=right width=50><form method=post action="/cgi-bin/siskop.cgi" >
      ...
      <input name="pages" type="hidden" value=kop022>
      <input name="ss" type="hidden" value="$in{ss}"> </form>
      </td>
     <td align=center> <h2 class="hurufcol">TAMBAH PENCAIRAN SIMPANAN </h2> </td>
...
where a.idlama=k.idlama and a.aktif!='Z' and a.idlama not in (select idlama from pinjaman where lunas='N'  and batal='N')
 order by nlengkap ");
$query->execute();
...
<form action="/cgi-bin/siskop.cgi" method="post" name='vq'>
  ...
  <input type=hidden name=pages value=kop022c>
<table>
  ...
  <tr bgcolor=$colcolor height=20>
    <td class="hurufcol" width=150 valign=top> &nbsp; Sampai Periode Akhir ? </td>
    <td class="hurufcol">~;
    if (!$in{rsg}){ $in{rsg}='N'; }
    for ($i=0; $i<@yesno; $i++)
    { $selected="";
      if($in{"rsg"} eq $yesno[$i]){ $selected='checked';}
      print qq~<input type=radio name=rsg value='$yesno[$i]' $selected> $yesno[$i]  &nbsp; &nbsp;  &nbsp; ~;
    }
  ...
  <tr bgcolor=$colcolor height=20>
    ...
    { $j=$i+1;
      $selected="";
      if($in{"simpanan$j"} eq $simp[$i] or $in{cair} eq 'all'){ $selected='checked';}
...
if($in{view})
{
  ...
  else
  {
    ...
    print qq~<table border=0 cellspacing=5 cellpadding=0 >
    <tr bgcolor=$colcolor height=20>
    ~;
    ...
    if($in{rsg} eq 'N')
    {
      $query = $dbk->prepare("select recid from periode where recid like '%12' order by recid desc rows 1 to 1 ");
      $query->execute();
      @rec = $query->fetchrow_array();
      $subq = " and periode<='$rec[0]' ";
    }
    ...
    {if($in{"simpanan$i"})
     { print qq~<td valign=top>
      ...
      $flag=$in{"simpanan$i"};
      $query = $dbk->prepare("select periode, hrsbayar, recid from iuran where idlama = $idlama
       and jenis='".$in{"simpanan$i"}."' and PENCAIRANid is null $subq ");
      $query->execute();
      $no=1; $jml=0; $ctr=1;
      while (@rec = $query->fetchrow_array())
      ...
      print qq~
        ...
        <input type=hidden name='totalsimp$flag' value=$jml>
...
if($in{simpan})
{
     ...
     $subq = "";   $nonaktif=0; $ctr=0;
     for ($i=1; $i<=2; $i++)
     { if($in{"simpanan$i"})
       {  $subq .= " or jenis='".$in{"simpanan$i"}."'";
          if($i==2){ $nonaktif=1; $qn = $dbk2->do("update anggota set aktif='N', oprupdate='$s3s[0]' where idlama = '$in{idlama}';");  }
          $ctr++;
       }
     }
     $subq = substr $subq,4;
     
     $query = $dbk->prepare("select max(recid) from PENCAIRAN ");
     $query->execute();
     @rec = $query->fetchrow_array();
     $recidm = $rec[0]+1;
     $ok=0;

#     $query = $dbk->prepare("select jenis, count(*), sum(hrsbayar) from iuran where idlama = $in{idlama} and PENCAIRANid is null
#     and ($subq)
#     group by jenis ");
#     $query->execute(); $no=1;
#     while (@rec = $query->fetchrow_array())
     ...
     for ($a=1; $a<=2; $a++)
     {if($in{"simpanan$a"})
      { $jnssimp = $in{"simpanan$a"};
#        print qq~INSERT INTO PENCAIRAN (RECID, PERIODE, IDLAMA, KETERANGAN, JUMLAH, DTTRANSAKSI, KATEGORI, OPRCREATE) VALUES
#        ('$recidm','$prid', $in{idlama}, '$jnssimp - ".$in{"maxctr$jnssimp"}."', ".$in{"totalsimp$jnssimp"}.", '$in{dtcair}',
#          'PS$jnssimp', '$s3s[0]' );<br/>~;
        $q = $dbk2->do("INSERT INTO PENCAIRAN (RECID, PERIODE, IDLAMA, KETERANGAN, JUMLAH, DTTRANSAKSI, KATEGORI, OPRCREATE) VALUES
        ('$recidm','$prid', $in{idlama}, '$jnssimp - ".$in{"maxctr$jnssimp"}."', ".$in{"totalsimp$jnssimp"}.", '$in{dtcair}',
          'PS$jnssimp', '$s3s[0]' );");
       if($q!=0)
       { $sukses=0;
         for($i=1; $i<=$in{"maxctr$jnssimp"}; $i++)
         { #print "update iuran set PENCAIRANid='$recidm', oprupdate='$s3s[0]' where recid='".$in{"recid$jnssimp$i"}."'; <br/>";
           $qd = $dbk2->do("update iuran set PENCAIRANid='$recidm', oprupdate='$s3s[0]' where recid='".$in{"recid$jnssimp$i"}."'");
           if($qd!=0){ $sukses++; }
         }
         
         if($sukses==$in{"maxctr$jnssimp"}){ $ok[$no]=1; }
         else { $ok[$no]=0; }
         #print qq~$sukses==$in{"maxctr$jnssimp"} $ok[$no]~;
       }
       $recidm++;
       $no++;
      }
     ...
     print qq~CTR= $ctr and $ok[1] and $ok[2] ~;
     if($ctr==1 and $ok[1]==1) { print qq~<div class=warning_ok>Sukses Pencairan Simpanan</div> ~; }
     elsif($ctr==2 and $ok[1]==1 and $ok[2]==1) { print qq~<div class=warning_ok>Sukses Pencairan Simpanan</div> ~; }

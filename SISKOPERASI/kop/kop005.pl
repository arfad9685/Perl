...
genfunckop::view_header_kop($in{ss}, $s3s[2], 'Master Menu', $s3s[17]);
genfunckop::validasi_akses_kop($s3s[11], $in{ss},'S');
&koneksi_kop2;

if ($s3s[11] eq 'S' and $in{kd}  && $s3s[7]==0)
{ $postall = $dbk->do("DELETE FROM X0000005  WHERE parent='$in{kd}'");
  $postall = $dbk->do("DELETE FROM X0000004  WHERE exeid='$in{kd}'");
  $postall = $dbk->do("DELETE FROM X0000002  WHERE namaexe='$in{kd}'");
  #$dbk->commit;
  $warning_del="<div class='warning_ok'>Sukses Hapus Menu </div><br/>";
}
if ($s3s[11] eq 'S' and $in{child}  && $s3s[7]==0)
{ $postall = $dbk->do("DELETE FROM X0000005  WHERE child='$in{child}'");
  #$dbk->commit;
...
{     print qq~<td align=right width=100>
      <!--<a href='/cgi-bin/colorbox.cgi?proses=str000&display=kop005a' class='iframe'>Add Menu</a>-->
      
      <form method=post action="/cgi-bin/siskop.cgi" >
      <input type="submit" name="back" value="Add Menu" class="huruf1"/>
      <input name="pages" type="hidden" value=kop005a>
...
<form action="/cgi-bin/siskop.cgi" method="post" name='vq'>
  ...
  <input type=hidden name=pages value=kop005>
<table>
...
$q = $dbk->prepare("select namamenu,  namaprog, namaexe, ex_in, lastprg from X0000001 a, X0000002 b WHERE a.recid=b.xid $subq order by namamenu, namaprog ");
$q->execute();
$no=1;
$tmpkat = '';
while (@row = $q->fetchrow_array())
{ $q2 = $dbk2->prepare("SELECT child, ketchild, parent  FROM  X0000005 WHERE parent='$row[2]' ORDER BY child ");
  $q2->execute();
  $div="<ul>";
  $j=1;

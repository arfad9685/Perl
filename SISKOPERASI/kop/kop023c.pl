...
<script type="text/javascript" src="/jscalendar/calendar-setup.js"></script>
     ...
     <td align=center> <h2 class="hurufcol"> TAMBAH TRANSAKSI PLUS</h2> </td>
...
if($in{simpan})
{
   ...
   else
   {
      ...
      print qq~INSERT INTO PLUS (periode, dttransaksi, keterangan, jumlah,  OPRCREATE) VALUES
                ('$periode','$in{dt}','$in{ket}','$in{jml}','$s3s[0]');~;
      
      $q1 = $dbk->do("INSERT INTO PLUS (periode, dttransaksi, keterangan, jumlah,  OPRCREATE) VALUES
                ('$periode','$in{dt}','$in{ket}','$in{jml}','$s3s[0]');");
      if($q1!=0)
      { print qq~<div class=warning_ok> Sukses Tambah Transaksi Plus </div> ~; }
      else
      { print qq~<div class=warning_not_ok> GAGAL Tambah Transaksi Plus : 1=$q1 </div> ~;  }
   }
}
else
{
...
print qq~<form action="/cgi-bin/kopbox.cgi" method="post" name='vq'>
  ...
  <input type=hidden name=display value=kop023c>
<table>

...
genfunckop::view_header_kop($in{ss}, $s3s[2], 'Edit Hak Akses User', $s3s[17]);
...
{  for ($i=1; $i<=$in{baris}; $i++)
   {
      ...
      {    if (!$in{"hak$i"}) { $exin=""; }
           ...
           if (!$in{"hak$i"})
           {
             $postall = $dbk->do("DELETE FROM X0000004  WHERE recid='".$in{"rec$i"}."' ");
           }
           else
           { # print qq~UPDATE X0000004 SET hwrite='".$in{"hak$i"}."', EX_IN='$exin', OPRUPDATE='$s3s[0]' WHERE recid='".$in{"rec$i"}."'<br/>~;
             $postall = $dbk->do("UPDATE X0000004 SET hwrite='".$in{"hak$i"}."', EX_IN='$exin', OPRUPDATE='$s3s[0]' WHERE recid='".$in{"rec$i"}."' ");
           }
      }
      elsif ($in{"hak$i"})
      {    $postall = $dbk->do("INSERT INTO X0000004 (NAMAUSR, EXEID, XID, HWRITE, EX_IN, OPRCREATE)
...
$q = $dbk->prepare("select  xid, namamenu, namaexe, namaprog, b.namausr, b.hwrite, recid from
(select  xid, namamenu, namaexe, namaprog from X0000002 k, X0000001 l
where k.xid=l.recid ) a
 left join
 (select m.namausr, hwrite, exeid, depid, recid from X0000004 m, getx3a n where m.namausr=n.namausr)
